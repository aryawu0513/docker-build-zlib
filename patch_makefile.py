...
makefile_out = "Makefile.in"

with open(makefile_in) as f:
    content = f.read()

# Fixed patterns - match the actual format with spaces between variables
...
def insert_after_block(pattern, text, content):
    ...
    if match:
        print(f"✓ Pattern matched!", flush=True)
        return re.sub(pattern, lambda m: m.group(0) + text, content, flags=re.MULTILINE)
    else:
        print(f"✗ Pattern NOT matched!", flush=True)
        return content
